...
const store = (set, get) => ({
	...
	deletePost: async id => {
		...
		try {
			...
			set({ posts: get().posts?.filter(post => post?._id !== id) }, false, 'delete-post')
	...
	likePost: async id => {
		set({ loading: true }, false, 'like-post')
		...
		try {
			...
			set({ loading: false }, false, 'like-post')
		}
	},

	savePost: async saves => {
		set({ loading: true }, false, 'save-post')
		const user = getUserLocalStorage()
		...
		try {
			...
			set({ loading: false }, false, 'save-post')
		}
	},
...
export const usePostsStore = create(
	devtools(immer(persist(store, { name: 'forito-posts' })))
)
